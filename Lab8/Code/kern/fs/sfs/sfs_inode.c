...
sfs_namefile(struct inode *node, struct iobuf *iob) {
    ...
    int ret;
    char *base = (char *)iob->io_base;
    size_t alen = 0;
    // 从后往前构建路径
    char path[iob->io_resid];
    int path_len = 0;
    ...
    while (1) {
        ...
        // 检查缓冲区是否足够（+1 为了 '/'）
        if (path_len + name_len + 1 > iob->io_resid) {
            goto failed_nomem;
        }
        ...
        memcpy(path + path_len, entry->name, name_len);
        path_len += name_len;
    }
    ...
    if (path_len == 0) {
        ...
        path_len = j;
        memcpy(path, temp, path_len);
    }
    
    // 复制到缓冲区
    if (path_len > iob->io_resid) {
        goto failed_nomem;
    }
    memcpy(base, path, path_len);
    base[path_len] = '\0';
    iobuf_skip(iob, path_len);
    kfree(entry);
    return 0;
