...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (uint32_t i = 0; i < elf.e_phnum; i++) {
        ...
        // (3.6) alloc memory, and copy the contents of every program section from file to process's memory
    ...
    // Calculate space for argv strings and pointers
    uint32_t argv_strs_size = argv_size;
    uint32_t argv_array_size = (argc + 1) * sizeof(uintptr_t);
    
    // Stack layout (from high to low): strings, argv array
    // Place strings starting from high address, going downward
    uint32_t argv_strs_start = stacktop - argv_strs_size;
    argv_strs_start = ROUNDDOWN(argv_strs_start, 4); // align to 4 bytes
    
    // Place argv array below strings
    uint32_t argv_array = argv_strs_start - argv_array_size;
    argv_array = ROUNDDOWN(argv_array, 4); // align to 4 bytes
    
    // Copy argv strings to user stack (from high to low)
    ...
    for (int i = 0; i < argc; i++) {
        ...
        // Get or allocate page for this address
...
bad_cleanup_current_mm:
    // (8) cleanup: restore lsatp and cleanup current->mm if steps after (5) failed
    lsatp(boot_pgdir_pa);
    if (mm_count_dec(mm) == 0) {
        exit_mmap(mm);
        put_pgdir(mm);
