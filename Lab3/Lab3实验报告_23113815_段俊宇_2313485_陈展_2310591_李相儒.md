...
&emsp;&emsp;使用$make{\quad}qemu$命令运行上面的代码，发现成功输出了$10$次$100ticks$，之后系统就停止了，说明关机成功
...
$1.$ $ trapentry.S$中汇编代码 $csrw\ sscratch,\ sp$；$csrrw\ s0,\ sscratch,\ x0$ 实现了什么操作，目的是什么？

  ```asm
  csrw sscratch, sp；	#将当前sp存入 sscratch，作为临时保存
  csrrw s0, sscratch, x0；	#取sscratch到 s0，同时将sscratch设为 0
  ```

​		我们知道，用户态下 $sp$ 指向用户栈，而内核处理中断时需要切换到内核栈，否则会覆盖用户栈内容。
 所以，进入中断处理时，必须安全地保存用户态 $sp$ 并切换到内核栈。

 		对于以上两条指令，单纯从实现的操作来说，$csrw\ sscratch,\ sp$ 是将当前栈指针 $sp$ 的值写入 $CSR\ sscratch$，
 $csrrw\ s0,\ sscratch,\ x0$ 是把 $sscratch$ 的值读入通用寄存器 $s0$，同时将 $sscratch$ 写入 $x0$（$x0$ 恒为 $0$），因此等同于将 $sscratch$ 清为 $0$。

​		而其目的主要有两点：一是将用户态的 $sp$ 安全放入寄存器 $s0$，避免在尚未调整 $sp$ 前将新 $sp$ 覆盖或干扰保存过程，以便稍后保存到 $trapframe$ 中；同时将 $sscratch$ 清零，作为处于内核模式或处理中断的一个标志。
...
```
  ...
  &emsp;&emsp;其中 $sstatus$ 和 $sepc$ 属于可读写的状态寄存器，$sstatus$ 包含中断使能、特权级等状态，$sepc$ 包含异常返回地址，用于 $sret$ 返回。$scause$、$sbadaddr$（$stval$）为只读的事件寄存器，其中 $scause$ 为异常原因寄存器，$sbadaddr$ 为错误地址寄存器。

​		所以将 $stval$、$scause$ 等 $CSR$ 保存到 $trapframe$，目的是让上层的 $C$ 函数 $trap(struct\ trapframe\ *tf)$ 能够读取这些值以判断异常类型、错误地址等，并据此作出打印、异常处理、进程终止等操作。 保存它们只是为了“上报”，而不是为了在返回时恢复原值。

​		同时，$scause$、$stval$ 等寄存器由硬件表示本次异常或中断的原因与相关信息，在处理完成后这些寄存器的值对继续执行被中断的程序没有影响，且写回它们没有任何必要。出于安全性考虑，一般只读取这些寄存器供内核进行决策，而不进行写回，因此使用这种 $store$ 方法。
