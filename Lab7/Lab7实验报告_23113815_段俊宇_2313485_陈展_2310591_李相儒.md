...
&emsp;&emsp;通过比较 Lab6 和 Lab7 的代码结构和关键文件，我们可以发现以下主要区别：

1.  **代码结构**：
    *   Lab7 在 `kern` 目录下新增了 `sync` 目录，其中包含了 `sem.c`, `sem.h` (信号量实现), `monitor.c`, `monitor.h` (管程实现), `check_sync.c` (同步问题测试，如哲学家就餐问题)。
    *   Lab6 主要关注调度器，在 `kern/schedule` 下实现了 `default_sched_stride.c` 等调度算法。

2.  **进程控制块 (`proc_struct`)**：
    *   Lab6 在 `proc_struct` 中增加了调度相关的字段，如 `lab6_run_pool`, `lab6_stride`, `lab6_priority`。
...
3.  **系统功能**：
    *   Lab6 的核心功能是实现更加公平和高效的 CPU 调度算法。
...
&emsp;&emsp;Lab7 中使用信号量实现了哲学家就餐问题 (`check_sync.c`)。
